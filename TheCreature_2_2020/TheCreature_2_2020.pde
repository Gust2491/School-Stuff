...
float   ballX             = 100 ,  ballY             = 100; //ændre til Float
float  eyeLeftRelativeX  = 5   ,  eyeLeftRelativeY  = 5; // ændrede int til float da int ikke kan være komma tal
float   eyeRightRelativeX = 20  ,  eyeRightRelativeY = 5; // ændrede til float  
float   speedX            = 1   ,  speedY            = 1; //ændre til float
float ballSize          = 50;
float eyeSize           = 5;
...
void draw() {                         
  float ballwalk =  sin(ballX*0.5)*10;  //svingning på +10 til -10 pixels //fjernede (//) 
  ballX         =  ballX  +  speedX;    //flytter min bold 
  ballY         =  ballY  +  speedY + ballwalk; //Satte ballwalk ind for at få den til at gå op og ned
